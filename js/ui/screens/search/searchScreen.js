...
export const SearchScreen = {
          ...
          <section class="search-header${this.layoutPrefs?.discoverLocation === "in_search" ? "" : " no-discover"}${showVoiceSearch ? "" : " no-voice"}">
            ${
              ...
              <button class="search-discover-btn focusable" data-action="openDiscover"
                aria-label="${escapeHtml(t("sidebar.discover", {}, "Discover"))}"
                title="${escapeHtml(t("sidebar.discover", {}, "Discover"))}">
                <span class="search-action-icon search-discover-icon" aria-hidden="true"></span>
              </button>
            `
                : ""
            }
  ...
  getDesktopHoverPreviewItem(node) {
    if (!node?.matches?.(".search-result-card.focusable[data-action='openDetail']")) {
      return null;
    }
    return {
      id: String(node.dataset.itemId || "").trim(),
      type: node.dataset.itemType || node.dataset.catalogType || "movie",
  ...
  bindDesktopMediaHoverPreview() {
    ...
    this.desktopMediaHoverPreview ||= createDesktopMediaHoverPreview({
      ...
      cardSelector: ".search-results-track .search-result-card.focusable[data-action='openDetail']",
      subtitleSelector: ".search-result-date"
    });
