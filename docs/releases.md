...
The publishing workflow rejects a tag that differs from `package.json`, runs tests and lint, builds and health-checks the whole Compose stack, then publishes all five images. Only after those jobs succeed does it create the GitHub release and attach a version-pinned Compose bundle and checksum. Existing release notes and assets are not overwritten on reruns.
...
The five GHCR packages must be public for anonymous self-hosting. GitHub initially creates new packages as private; check package visibility when adding a service. Published images currently target `linux/amd64`; ARM64 remains available through a source build.
